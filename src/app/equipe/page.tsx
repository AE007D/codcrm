...
export default function EquipePage() {
  ...
  // productId → { commissionAmount, boutiqueNom, checked }
  const [assignForm, setAssignForm] = useState<Record<string, { checked: boolean; commissionAmount: string; boutiqueNom: string }>>({});
  ...
  async function openAssign(agent: TeamUser) {
  ...
  async function saveAssign() {
    ...
    try {
      await Promise.all(
        allProducts.map(p => {
          const f = assignForm[p.id];
          const shouldAssign = f?.checked;
          const wasAssigned = p.agentId === assignAgent.id;
          if (shouldAssign === wasAssigned && shouldAssign === false) return Promise.resolve();
          ...
          return fetch("/api/products", {
            ...
            body: JSON.stringify({
              id: p.id,
              agentId: shouldAssign ? assignAgent.id : "",
              agentName: shouldAssign ? assignAgent.name : "",
              commissionAmount: shouldAssign ? parseFloat(f.commissionAmount || "0") || 0 : 0,
              boutiqueNom: shouldAssign ? (f.boutiqueNom || "") : (p.boutiqueNom ?? ""),
            }),
          });
        })
      );
  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    try {
      ...
      setInviteName(""); setInviteEmail(""); setInvitePassword(""); setInviteRole("agent");
      await fetchUsers();
    } catch {
  ...
  async function handleRoleChange(userId: string, role: UserRole) {
    ...
    try {
      await fetch("/api/auth/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: userId, role }) });
      await fetchUsers();
  ...
  async function handleToggleActive(user: TeamUser) {
    ...
    try {
      await fetch("/api/auth/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: user.id, active: !user.active }) });
      await fetchUsers();
    } finally { setActionLoading(null); }
  }
  ...
  async function handleDelete(userId: string) {
    ...
    try {
      await fetch("/api/auth/users", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: userId }) });
      await fetchUsers();
    } finally { setActionLoading(null); }
  }
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="font-semibold text-slate-800">
                                    {u.name}
                                    {isSelf && <span className="ml-2 text-xs text-slate-400 font-normal">(vous)</span>}
                                  </p>
      ...
      {showInviteModal && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            {inviteError && (
              <div className="bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-xl mb-4">{inviteError}</div>
            )}
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                <h2 className="text-base font-bold text-slate-900">Produits assignés</h2>
                <p className="text-xs text-slate-400 mt-0.5">{assignAgent.name}</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {assignLoading ? (
                ...
                <div className="flex flex-col gap-3">
                  {allProducts.map(p => {
                    const f = assignForm[p.id] ?? { checked: false, commissionAmount: "", boutiqueNom: "" };
                    return (
                      <div key={p.id} className={`rounded-2xl border p-4 transition-colors ${f.checked ? "border-emerald-200 bg-emerald-50/40" : "border-slate-100 bg-white"}`}>
                        <div className="flex items-center gap-3">
                          <input type="checkbox" checked={f.checked}
                            onChange={e => setAssignForm(prev => ({ ...prev, [p.id]: { ...f, checked: e.target.checked } }))}
                        ...
                        {f.checked && (
                          <div className="mt-3 grid grid-cols-2 gap-2 pl-7">
                            <div>
                              ...
                              <input type="text" placeholder="Ex: Atlas Store" value={f.boutiqueNom}
                                onChange={e => setAssignForm(prev => ({ ...prev, [p.id]: { ...f, boutiqueNom: e.target.value } }))}
                            ...
                            <div>
                              ...
                              <input type="number" placeholder="15" min="0" value={f.commissionAmount}
                                onChange={e => setAssignForm(prev => ({ ...prev, [p.id]: { ...f, commissionAmount: e.target.value } }))}
                                className="w-full text-xs border border-emerald-200 rounded-xl px-3 py-2 outline-none focus:border-emerald-400 bg-white" />
                            </div>
                          </div>
                        )}
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
              ...
              <button onClick={saveAssign} disabled={assignSaving || assignLoading}

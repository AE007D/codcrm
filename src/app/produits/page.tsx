...
export default function ProduitsPage() {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    // Compress to max 800px JPEG 80%, then upload to Supabase Storage
    ...
    img.onload = () => {
      ...
      canvas.toBlob(async (blob) => {
        if (!blob) { setFormError("Erreur compression image."); return; }
        const fd = new FormData();
        fd.append("file", blob, "product.jpg");
        setFormError(""); // clear errors
        setForm(f => ({ ...f, image: "__uploading__" })); // show spinner
        try {
          const res = await fetch("/api/upload", { method: "POST", body: fd });
          const data = await res.json();
          if (!res.ok || !data.url) { setFormError(data.error || "Échec upload image."); setForm(f => ({ ...f, image: "" })); return; }
          setForm(f => ({ ...f, image: data.url }));
        } catch {
          setFormError("Erreur réseau lors de l'upload.");
          setForm(f => ({ ...f, image: "" }));
        }
      }, "image/jpeg", 0.8);
    };
    img.onerror = () => { URL.revokeObjectURL(objectUrl); setFormError("Impossible de lire cette image."); };
    img.src = objectUrl;
  }
                  ...
                  <div key={p.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex flex-col">
                    ...
                    <div className="p-4 flex flex-col gap-3 flex-1">
                      ...
                      <div className="flex gap-2 mt-auto flex-col">
                        ...
                        <button onClick={() => copyPageLink(p.id)}
                          className={`w-full flex items-center justify-center gap-1.5 text-xs font-semibold py-2 rounded-xl border transition-colors ${copiedId === p.id ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "border-violet-200 text-violet-600 hover:bg-violet-50"}`}>
                          {copiedId === p.id ? (
                            <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} className="w-3.5 h-3.5"><polyline points="20 6 9 17 4 12"/></svg>Lien copié !</>
                          ) : (
                            <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>Copier lien page</>
                          )}
                        </button>
                      </div>
                    </div>

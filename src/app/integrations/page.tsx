...
export default function IntegrationsPage() {
  ...
  async function addAmeex() { if (!ameexSaved) { showToast("Configurez Ameex d'abord.", false); return; } if (!ameexForm.receiver || !ameexForm.phone || !ameexForm.city || !ameexForm.address || !ameexForm.cod) { showToast("Champs obligatoires manquants.", false); return; } setLoading(true); const d = await ameexCall("addParcel", ameexSaved, { ...ameexForm }); setLoading(false); const ok = d?.status === "success" || d?.success === true || d?.id != null || d?.tracking != null || d?.barcode != null || d?.parcel_id != null || (typeof d?.message === "string" && d.message.toLowerCase().includes("success")); const label = d?.id ? `Colis créé ✓ — ID: ${d.id}` : d?.tracking ? `Colis créé ✓ — Tracking: ${d.tracking}` : d?.message || (ok ? "Colis créé ✓" : `Erreur: ${JSON.stringify(d)}`); showToast(label, ok); if (ok) setAmeexForm({ receiver: "", phone: "", city: "", address: "", cod: "", product: "", order_num: "", comment: "", type: "SIMPLE", open: "NO", try: "NO", fragile: "0", replace: "false" }); }

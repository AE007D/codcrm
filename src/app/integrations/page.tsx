...
export default function IntegrationsPage() {
  ...
  async function addAmeex() { if (!ameexSaved) { showToast("Configurez Ameex d'abord.", false); return; } if (!ameexForm.receiver || !ameexForm.phone || !ameexForm.city || !ameexForm.address || !ameexForm.cod) { showToast("Champs obligatoires manquants.", false); return; } setLoading(true); const d = await ameexCall("addParcel", ameexSaved, { ...ameexForm }); setLoading(false); const nested = d?.api?.data ?? d?.data ?? d; const ok = d?.login === "success" || d?.api?.type === "success" || nested?.id != null || nested?.code != null || d?.status === "success" || d?.success === true || d?.id != null || (typeof (d?.api?.msg ?? d?.message) === "string" && (d?.api?.msg ?? d?.message).toLowerCase().includes("succès")); const trackCode = nested?.code ?? nested?.id ?? d?.id ?? d?.tracking ?? d?.barcode; const label = ok ? `Colis créé ✓ — ${trackCode ?? d?.api?.msg ?? "OK"}` : `Erreur: ${JSON.stringify(d)}`; showToast(label, ok); if (ok) setAmeexForm({ receiver: "", phone: "", city: "", address: "", cod: "", product: "", order_num: "", comment: "", type: "SIMPLE", open: "NO", try: "NO", fragile: "0", replace: "false" }); }
